namespace weighted_blossom_tree{ // n 400 w 1e8 700ms, n 500 w 1e6 300ms
  ...
  bool bfs(){
    ...
    while (true){
      while (h <= t){
        int u = q[h++];
        if (s[st[u]] != 1) for (int v=1; v<=n; v++) if (e[u][v].w > 0 && st[u] != st[v])
          if(d(e[u][v])) upd(u, st[v]); else if(on(e[u][v])) return true;
      }
      T x = inf;
      for(int i=n+1; i<=m; i++) if(st[i] == i && s[i] == 1) x = min(x, lab[i]>>1);
      for(int i=1; i<=m; i++) if(st[i] == i && sl[i] && s[i] != 1) x = min(x, d(e[sl[i]][i])>>s[i]+1);
      for(int i=1; i<=n; i++) if(~s[st[i]]) if((lab[i] += (s[st[i]]*2-1)*x) <= 0) return false;
      for(int i=n+1 ;i<=m; i++) if(st[i] == i && ~s[st[i]]) lab[i] += (2-s[st[i]]*4)*x;
      h = 1; t = 0;
  ...
  template<typename TT> pair<int,ll> run(int N, const vector<tuple<int,int,TT>> &edges){ // 1-based
    ...
    for(int i=1; i<=n; i++) for (int j=1; j<=n; j++) b[i][j] = i*(i==j);
