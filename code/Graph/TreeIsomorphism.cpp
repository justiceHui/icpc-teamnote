struct Tree{ // (M1,M2)=(1e9+7, 1e9+9), P1,P2 = random int array(sz >= N+2)
  ...
  pair<int,int> build(const vector<ll> &P1, const vector<ll> &P2, int v, int b=-1){
    ...
    ll h1 = 0, h2 = 0; stable_sort(ch.begin(), ch.end()); if(ch.empty()){ return {1, 1}; }
    for(int i=0; i<ch.size(); i++) h1=(h1+(ch[i].first^P1[P1.size()-1-i])*P1[i])%M1, h2=(h2+(ch[i].second^P2[P2.size()-1-i])*P2[i])%M2;
    return H[v] = {h1, h2};
  }
